 ...
 * NAPRAWKI: String comparison dla echo filter, zwiększony timeout key exchange
 */
class ChatManager {
  constructor() {
    // Core properties
    ...
    console.log("✅ Optimized ChatManager initialized (Real-time only):", this.user.username);
    ...
    setTimeout(() => {
      ...
      if (rtStatus.status === 'real-time') {
        ...
        this._showNotification('⚠️ Using polling backup', 'warning', 3000);
      } else {
        this._showNotification('❌ Messaging offline - trying to reconnect', 'error', 3000);
  ...
  // === INITIALIZATION ===
  _loadUserData() {
    // NAPRAWKA: Zunifikowane identyfikatory użytkowników z String() conversion
    const rawUserId = sessionStorage.getItem('user_id');
    const userData = {
      id: rawUserId ? parseInt(rawUserId) : null,  // INT dla porównań (będzie konwertowane do String)
      user_id: rawUserId,                          // STRING ID z serwera
    ...
    // DODANE: Walidacja danych użytkownika
    if (!userData.id || !userData.user_id) {
      console.error("❌ Invalid user data - redirect to login");
  ...
  async _initSocket() {
    try {
      ...
      setTimeout(() => this._enablePollingFallback(), 2000);
    }
  }
  ...
  _setupSocketEvents() {
    this.socket.on('connect', () => {
      ...
      // Join current session room if available
      if (this.currentSession) {
        this.socket.emit('join_session', { session_token: this.currentSession.token });
      }
    });

    this.socket.on('disconnect', (reason) => {
      console.log(`🔌 Socket.IO disconnected: ${reason}`);
      this._showNotification('Connection lost - reconnecting...', 'warning', 2000);
    });

    this.socket.on('reconnect', () => {
      console.log("🔌 Socket.IO reconnected");
      this._showNotification('✅ Connection restored', 'success', 2000);
      
      // Re-register user and rejoin session
      this.socket.emit('register_user', { user_id: this.user.id });
      if (this.currentSession) {
        this.socket.emit('join_session', { session_token: this.currentSession.token });
      }
    ...
    this.socket.on('connect_error', (error) => {
      ...
      // Fallback to polling if Socket.IO fails repeatedly
      setTimeout(() => this._enablePollingFallback(), 5000);
    });
  }
  ...
  async _handleNewMessage(data) {
    ...
    // === NAPRAWIONY ECHO FILTER - CRITICAL FIX ===
    const messageSenderId = String(data.message.sender_id); // ZAWSZE STRING
    const currentUserId = String(this.user.id);             // ZAWSZE STRING
    
    console.log("🔍 Echo check:", messageSenderId, "vs", currentUserId, "(both strings now)");
  ...
  async _performKeyRetrieval(sessionToken) {
    ...
    try {
      ...
      let attempts = 0;
      const maxAttempts = 20; // ZWIĘKSZONE z 10 na 20
      ...
      while (!encryptedKey && attempts < maxAttempts) {
        ...
        // Wait 2 seconds before retry (ZWIĘKSZONE z 1 sekundy)
      ...
      if (!encryptedKey) {
        throw new Error('Timeout: No session key from initiator after 20 attempts (40 seconds)');
      }
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.status === 'success' && data.messages.length > 0) {
            console.log(`📨 Polling: ${data.messages.length} new messages`);
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        if (this.socket && this.socket.connected) {
          this.socket.emit('join_session', { session_token: data.session.token });
        }
