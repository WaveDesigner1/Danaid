"""
auth.py - Oryginalny sposób logowania/rejestracji + minimalne poprawki dla kompatybilności
"""
import json
import secrets
import base64
...
def register():
    """Rejestracja użytkownika - ORYGINALNY SPOSÓB"""
    try:
...
def login():
    """Logowanie użytkownika - ORYGINALNY SPOSÓB + minimalne poprawki"""
    try:
        data = request.get_json()
        username = data.get('username')
        password = data.get('password')
        ...
        # 🔧 MINIMALNA POPRAWKA: Aktualizuj nowe kolumny (jeśli istnieją)
        try:
        ...
        # Zapisz czas dla session heartbeat (ORYGINALNY SPOSÓB)
...
@login_required
def logout():
    """Wylogowanie użytkownika - ORYGINALNY SPOSÓB + minimalne poprawki"""
    try:
        print(f"🔓 Logging out user: {current_user.username}")
        
        # 🔧 MINIMALNA POPRAWKA: Aktualizuj status offline (jeśli kolumna istnieje)
        try:
...
def logout_redirect():
    """Przekierowanie wylogowania - ORYGINALNY SPOSÓB"""
    try:
    ...
    # Clear cookies (ORYGINALNY SPOSÓB)
...
# === DODATKOWE API (jeśli były w oryginale) ===
...
# === DEBUG ENDPOINTS (opcjonalne, do usunięcia w produkcji) ===
