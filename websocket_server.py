...
class WebSocketHandler:
    ...
    def send_to_user(self, user_id, message):
        """Wyślij wiadomość do użytkownika (synchroniczna wersja dla chat_api)"""
        if not user_id or user_id not in active_connections:
            logger.info(f"Użytkownik {user_id} nie jest połączony")
            return False
        
        # Uruchom asynchroniczną funkcję w nowej pętli zdarzeń
        try:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            result = loop.run_until_complete(send_to_user(user_id, message))
            loop.close()
            return result
        except Exception as e:
