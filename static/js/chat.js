 ...
 * NAPRAWIONO: RSA decryption error handling i debugging + RETRY LOGIC
 */
class CryptoManager {
  constructor() {
  ...
  // === NAPRAWIONE DESZYFROWANIE RSA Z RETRY LOGIC ===
  async decryptSessionKey(encryptedKey) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      console.log(`🔓 RSA decryption attempt ${attempt}/${maxRetries}`);
      
      try {
        return await this._performDecryption(encryptedKey);
      } catch (error) {
        lastError = error;
    ...
    // All attempts failed
    console.error(`❌ RSA decryption failed after ${maxRetries} attempts`);
    throw new Error(`RSA decryption failed after ${maxRetries} attempts: ${lastError.message}`);
  }

  async _performDecryption(encryptedKey) {
  ...
  getSecurityInfo() {
    ...
    return {
      ...
      debugging: true,
      retryLogic: true  // DODANE: wskazuje że retry logic jest aktywny
    };
  }
  ...
  // === DODANE: ENHANCED ERROR RECOVERY ===
  async recoverFromDecryptionError(encryptedKey, originalError) {
    console.log("🔄 Attempting error recovery for RSA decryption...");
    
    try {
      // Spróbuj ponownie załadować klucz prywatny
      console.log("🔄 Reloading private key...");
      const privateKeyPEM = sessionStorage.getItem('private_key_pem');
      if (privateKeyPEM) {
        this.privateKey = await this._importPrivateKey(privateKeyPEM);
        console.log("✅ Private key reloaded");
        
        // Spróbuj deszyfrowania ponownie
        return await this._performDecryption(encryptedKey);
      } else {
        throw new Error("No private key available for recovery");
      }
    } catch (recoveryError) {
      console.error("❌ Recovery failed:", recoveryError.message);
      throw new Error(`Original error: ${originalError.message}, Recovery failed: ${recoveryError.message}`);
    }
  }

  // === DODANE: NETWORK ERROR RETRY ===
  ...
  async performHealthCheck() {
    console.log("🏥 Performing crypto health check...");
    
    const results = {
      privateKeyLoaded: !!this.privateKey,
      publicKeyLoaded: !!this.publicKey,
      sessionKeysCount: this.sessionKeys.size,
      storageKeysCount: Object.keys(sessionStorage).filter(k => k.startsWith('session_key_')).length,
    ...
    try {
      ...
      const encrypted = await this.encryptMessage(testKey, testData);
      const decrypted = await this.decryptMessage(testKey, encrypted);
      
      results.basicCryptoWorking = (decrypted === testData);
      console.log("✅ Basic crypto operations working");
    } catch (error) {
      results.basicCryptoWorking = false;
      results.errors.push(`Basic crypto test failed: ${error.message}`);
...
window.cryptoHealthCheck = () => window.cryptoManager.performHealthCheck();
